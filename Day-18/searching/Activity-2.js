...
function binarySearch(arr, target) {
  ...
  while (left <= right) {
    ...
    if (arr[mid] === target) {
      ...
      // If the middle element is less than the target value, update the left pointer
      left = mid + 1;
    } else {
      // If the middle element is greater than the target value, update the right pointer
      right = mid - 1;
    }
  }
